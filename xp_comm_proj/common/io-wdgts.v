...
flibrary IOWidgets <compile_subs=0> {
  ...
  XP_COMM_PROJ.Common.UIWidgets.IAC_StandardUI CreateIndexedFilenameUI<NEx=55.,NEy=77.> {

      XP_COMM_PROJ.Common.IOWidgets.IndexedFileParams &IndexedFileParams<NEx=198.,NEy=11.,NEportLevels={2,1}>;
      ...
      XP_COMM_PROJ.Common.UIWidgets.SmartTextTypein DirectorySmartTextTypein<NEx=462.,NEy=88.> {
         UIparent => <-.panel;
         slabel => <-.dirString;
         stext => <-.IndexedFileParams.dir;
      };
      XP_COMM_PROJ.Common.UIWidgets.SmartTextTypein StubSmartTextTypein<NEx=462.,NEy=132.> {
         UIparent => <-.panel;
         slabel => <-.stubString;
         stext => <-.IndexedFileParams.filename_stub;
      };
      XP_COMM_PROJ.Common.UIWidgets.SmartTextTypein ExtSmartTextTypein<NEx=462.,NEy=176.> {
         UIparent => <-.panel;
         slabel => <-.extString;
         stext => <-.IndexedFileParams.extension;
      };

      XP_COMM_PROJ.Common.UIWidgets.SmartFieldTypein SmartFieldTypein<NEx=462.,NEy=220.> {
         UIparent => <-.panel;
         flabel => <-.widthString;
  ...
  macro createIndexedFilename<NEx=187.,NEy=88.> {
      ...
      XP_COMM_PROJ.Common.IOWidgets.IndexedFileParams IndexedFileParams<NEx=396.,NEy=77.> {
         index_width = 0;
      };
      XP_COMM_PROJ.Common.IOWidgets.CreateIndexedFilename CreateIndexedFilename<NEx=187.,NEy=165.> {
         FileParams => <-.IndexedFileParams;
         index => <-.index;
      };
      XP_COMM_PROJ.Common.IOWidgets.CreateIndexedFilenameUI CreateIndexedFilenameUI<NEx=374.,NEy=165.> {
  ...
  //   Used to setup and alter the AnimLoopParams group.
   ...
   XP_COMM_PROJ.Common.UIWidgets.IAC_StandardUI AnimFilenameUI {
      ...
      XP_COMM_PROJ.Common.UIWidgets.titleLabel titleLabel<NEx=132.,NEy=275.> {
         parent => <-.panel;
      ...
      XP_COMM_PROJ.Common.UIWidgets.VideoLoopParams &VideoLoopParams<NEx=539.,NEy=385.,NEportLevels={2,1}>;

      XP_COMM_PROJ.Common.UIWidgets.SmartFieldTypein StartSmartFieldTypein<NEx=473.,NEy=132.> {
         UIparent => <-.panel;
         flabel => <-.startString;
         fval => <-.VideoLoopParams.start_val;
      };
      XP_COMM_PROJ.Common.UIWidgets.SmartFieldTypein EndSmartFieldTypein<NEx=473.,NEy=176.> {
         UIparent => <-.panel;
         flabel => <-.endString;
         fval => <-.VideoLoopParams.end_val;
      };
      XP_COMM_PROJ.Common.UIWidgets.SmartFieldTypein IncrSmartFieldTypein<NEx=473.,NEy=220.> {
         UIparent => <-.panel;
         flabel => <-.incrString;
      ...
      XP_COMM_PROJ.Common.UIWidgets.VideoControl VideoControl<NEx=836.,NEy=341.> {
         UIparent => <-.panel;
  ...
  macro AnimFilename {
    ...
    XP_COMM_PROJ.Common.IOWidgets.IndexedFileParams &IndexedFileParams<NEx=22.,NEy=77.,NEportLevels={2,1}>;
    XP_COMM_PROJ.Common.UIWidgets.VideoLoopParams &VideoLoopParams<NEx=176.,NEy=33.,NEportLevels={2,1}>;

    XP_COMM_PROJ.Common.UIWidgets.VideoLoop VideoLoop<NEx=209.,NEy=88.> {
        VideoLoopParams => <-.VideoLoopParams;
    };

    XP_COMM_PROJ.Common.IOWidgets.CreateIndexedFilename CreateIndexedFilename<NEx=99.,NEy=165.> {
        FileParams => <-.IndexedFileParams;
  ...
  macro animFilename {
    XP_COMM_PROJ.Common.UIWidgets.VideoLoopParams VideoLoopParams <NEx=22.,NEy=22.> {
      start_val = 0;
      end_val = 10;
      incr = 1;
    };

    XP_COMM_PROJ.Common.IOWidgets.IndexedFileParams IndexedFileParams<NEx=22.,NEy=66.> {
      dir = "build3d/";
      filename_stub = "cube";
      extension = "tif";
      index_width = 0;
    ...
    XP_COMM_PROJ.Common.IOWidgets.AnimFilenameUI AnimFilenameUI <NEx=286.,NEy=99.> {
      VideoLoopParams => <-.VideoLoopParams;
      UImod_panel {
        title => name_of(<-.<-.<-);
      };
    ...
    XP_COMM_PROJ.Common.IOWidgets.CreateIndexedFilenameUI CreateIndexedFilenameUI<NEx=22.,NEy=143.> {
      IndexedFileParams => <-.IndexedFileParams;
    };

    XP_COMM_PROJ.Common.IOWidgets.AnimFilename AnimFilename<NEx=198.,NEy=143.> {
      VideoLoopParams => <-.VideoLoopParams;
      IndexedFileParams => <-.IndexedFileParams;
    };
