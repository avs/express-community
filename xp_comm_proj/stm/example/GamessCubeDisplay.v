APPS.SingleWindowApp MolDisplay_GAMESScube {
   UI {
      shell {
         x = 152;
         y = 20;
         ...
         IUI {
            optionList {
               cmdList => {
                  ...
                  <-.<-.<-.<-.Coord_To_Geom_Display.readGamessCube.isosurface.UIpanel.option,
                  <-.<-.<-.<-.Coord_To_Geom_Display.readGamessCube.bounds.UIpanel.option,
                  <-.<-.<-.<-.Coord_To_Geom_Display.readGamessCube.orthoslice.OrthoSliceUI.UIpanel.option,
                  <-.<-.<-.<-.Coord_To_Geom_Display.readGamessCube.isoline.UIpanel.option,
                  <-.<-.<-.<-.Coord_To_Geom_Display.readGamessCube.extract_scalar.ExtractScalarUI.UIpanel.option
               };
               selectedItem = 2;
            };
         };
      ...
      Scene {
         Top {
            child_objs => {
               ...
               <-.<-.<-.Coord_To_Geom_Display.readGamessCube.isosurface.out_obj,
               <-.<-.<-.Coord_To_Geom_Display.readGamessCube.isoline.out_obj,
               <-.<-.<-.Coord_To_Geom_Display.readGamessCube.bounds.out_obj
            };
            Props {
               subdiv = 15;
               line_aa = 1;
...
26.07};
            ...
            Xform {
               mat = {
                  -0.438858,0.0754366,-0.0717939,0.,0.000876294,-0.30827,-0.329251,
0.,-0.104138,-0.320499,0.299794,0.,0.,0.,0.,1.
               };
               ocenter = {-0.338478,-0.225249,
-0.470673};
               dcenter = {-0.2375,-0.15805,
-0.330256};
               xlate = {0.2375,0.15805,0.330256};
               center = {-0.338478,-0.225249,
-0.470673};
            };
            Modes {
               outline = "Off";
...
0.,-0.254369,-0.485395,0.836474,0.,0.,0.,0.,1.
   ...
   macro Coord_To_Geom_Display<NEx=165.,NEy=11.,NEscalingFactor=1.,NExOffset=0.,NEyOffset=0.> {
      APPS.ModuleStack MolDisplayUI<NEx=44.,NEy=11.,NEscalingFactor=0.8168498278,NExOffset=-11.,NEyOffset=63.39461899> {
         ModuleStackUI {
            mod_shell {
               y = 59;
               width => 391;
         ...
         UIlist File_Type<NEx=11.,NEy=165.,NEwidth=506.,NEheight=220.> {
            ...
            parent => <-.File_Type_Frame;
            message<NEportLevels={2,0}> => <-.File_Type_Frame.message;
            selectedItem<NEportLevels={0,3}> = 5;
            stringdims => 14;
            strings = {
      ...
      macro MolNet<NExOffset=155.0147095,NEyOffset=-44.,NEscalingFactor=0.8717948794,NEx=11.,NEy=220.> {
         macro atomic_positions<NEx=-143.,NEy=88.,NEwidth=484.,NEheight=198.> {
...
50.};
         ...
         macro atomic_bonds<NEx=11.,NEy=88.,NEwidth=330.,NEheight=154.> {
...
0.039991};
         ...
         MODS.text_glyph residue_name_text<NEx=44.,NEy=132.> {
         ...
         MODS.text_glyph atom_name_text<NEx=220.,NEy=132.> {
            in_mesh => <-.atomic_positions.atomic_point_mesh.out;
      ...
      macro readGamessCube<NEx=418.,NEy=99.> {
         FLD_MAP.struct_mesh struct_mesh<NEx=297.,NEy=55.> {
            in_dims<NEportLevels={3,0}> => <-.<-.ReadGamessCube.dims;
            coord<NEportLevels={3,0}> => <-.<-.ReadGamessCube.Coords;
         };
         ...
         MODS.isosurface isosurface<NEx=187.,NEy=187.> {
            ...
            IsoParam {
               iso_level<NEdisplayMode="open"> => -0.031;
            };
            DVcell_data_labels {
               labels[];
            };
...
50.};
         ...
         MODS.isoline isoline<NEx=132.,NEy=253.> {
            in_field => <-.orthoslice.out_fld;
            IsoParam {
               ncontours<NEdisplayMode="open"> = 45;
               level_min<NEdisplayMode="open"> => -0.19;
               level_max<NEdisplayMode="open"> => 0.19;
            };
            obj {
               Datamap {
                  DataRange = {
         ...
         FLD_MAP.node_vector node_vector<NEx=11.,NEy=55.> {
            in_data<NEportLevels={3,0}> => <-.<-.ReadGamessCube.Data;
         };
      ...
      XP_COMM_PROJ.STM.MODULE.Mol_c Mol_c<NEx=33.,NEy=154.> {
         ...
         File_Name => <-.MolDisplayUI.UIfileBrowser.file_name;
      ...
      XP_COMM_PROJ.STM.MODULE.ReadGamessCube ReadGamessCube<NEx=374.,NEy=33.> {
         FileName => <-.MolDisplayUI.UIfileBrowser.file_name;
         TotalSize<NEportLevels={0,1}>;
         trigger => <-.MolDisplayUI.Trigger;
      };
   };
};
