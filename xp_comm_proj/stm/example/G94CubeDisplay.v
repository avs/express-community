APPS.SingleWindowApp MolDisplay_GaussianCube {
   UI {
      shell {
         x = 289;
         y = 325;
         ...
         IUI {
            optionList {
               cmdList => {
                  ...
                  <-.<-.<-.<-.Coord_To_Geom_Display.readCube.isosurface.UIpanel.option,
                  <-.<-.<-.<-.Coord_To_Geom_Display.readCube.bounds.UIpanel.option,
                  <-.<-.<-.<-.Coord_To_Geom_Display.readCube.orthoslice.OrthoSliceUI.UIpanel.option,
                  <-.<-.<-.<-.Coord_To_Geom_Display.readCube.isoline.UIpanel.option
               };
               selectedItem = 2;
            };
            panel {
      ...
      status_bar {
         y<NEx=308.,NEy=33.,NEwidth=330.,NEheight=55.>;
      };
   };
      ...
      Scene {
         Top {
            child_objs => {
               ...
               <-.<-.<-.Coord_To_Geom_Display.readCube.isosurface.out_obj,
               <-.<-.<-.Coord_To_Geom_Display.readCube.isoline.out_obj,
               <-.<-.<-.Coord_To_Geom_Display.readCube.bounds.out_obj
            };
            Props {
               subdiv = 15;
               line_aa = 1;
...
26.07};
            ...
            Xform {
               mat = {
                  -0.15039,0.162486,0.394343,0.,-0.0384391,0.411274,-0.184123,
0.,-0.424772,-0.0947459,-0.122955,0.,0.,0.,0.,1.
               };
               ocenter = {0.0393457,0.288575,
-0.0403681};
               dcenter = {0.0177939,0.130507,
-0.0182563};
               xlate = {-0.0177939,-0.130507,
0.0182562};
               center = {0.0393457,0.288575,
-0.0403681};
            };
            Modes {
               outline = "Off";
...
0.,-0.254369,-0.485395,0.836474,0.,0.,0.,0.,1.
   ...
   macro Coord_To_Geom_Display<NEx=165.,NEy=11.,NEscalingFactor=1.,NExOffset=0.,NEyOffset=0.> {
      APPS.ModuleStack MolDisplayUI<NEx=11.,NEy=11.,NEscalingFactor=0.8168498278,NExOffset=-11.,NEyOffset=63.39461899> {
         ModuleStackUI {
            mod_shell {
               y = 59;
               width => 391;
...
0.039991};
      ...
      XP_COMM_PROJ.STM.MODULE.Mol_c Mol_c<NEx=11.,NEy=110.,NEwidth=275.,NEheight=627.> {
      ...
      macro readCube<NEx=418.,NEy=77.> {
         XP_COMM_PROJ.STM.MODULE.Read_G94_cube Read_G94_cube<NEx=451.,NEy=11.> {
            File_Name<NEportLevels={3,0}> => <-.<-.MolDisplayUI.UIfileBrowser.file_name;
         };
         ...
         MODS.isosurface isosurface<NEx=352.,NEy=154.> {
            ...
            IsoParam {
               iso_level<NEdisplayMode="open"> => 0.001;
            };
            DVcell_data_labels {
               labels[];
            };
...
50.};
         ...
         MODS.isoline isoline<NEx=242.,NEy=198.> {
            ...
            IsoParam {
               ncontours<NEdisplayMode="open"> = 13;
               level_min<NEdisplayMode="open"> => -0.077;
               level_max<NEdisplayMode="open"> => 0.172;
            };
            obj {
               Datamap {
                  DataRange = {
