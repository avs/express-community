...
// global error message to display errors not only on stdout
char v5d_error_msg[256];
...
#define SKIP(N)   lseek( f, N, SEEK_CUR )
   ...
   if (id==TAG_ID && idlen==0) {
	  ...
	  strcpy(v5d_error_msg, "Error: not a v5d file");
   ...
   while (!end_of_header) {
      ...
      if (read_int4(f,&tag)<1 || read_int4(f,&length)<1) {
		 ...
		 strcpy(v5d_error_msg, "Error while reading header, premature EOF");
         return 0;
      }
      ...
      switch (tag) {
         case TAG_VERSION:
            ...
            if (strcmp(v->FileVersion, FILE_VERSION)>0) {
			   ...
			   sprintf(v5d_error_msg, "Warning: Trying to read a version %s file, you should upgrade", v->FileVersion);
            }
            break;
         case TAG_NUMTIMES:
            assert( length==4 );
...
v5dstruct *v5dOpenFile( const char *filename, v5dstruct *v )
{
   ...
   if (v) {
      ...
      v = v5dNewStruct();
      if (!v) {
		  strcpy(v5d_error_msg, "Cannot allocate structure");
