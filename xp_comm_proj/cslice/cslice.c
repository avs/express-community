...
int
Slice_Coords(OMobj_id Coords_Slice_id, OMevent_mask event_mask, int seq_num)
{
	/***********************/
	/*	 Declare variables  */
	/***********************/
	...
	if (OMget_name_real_val(Coords_Slice_id, OMstr_to_name("plane"), &plane) != 1) 
		plane = 0.0;

	if (OMget_name_int_val(Coords_Slice_id, OMstr_to_name("axis"), &axis) != 1) 
		axis = 0;
		
	if (OMget_name_int_val(Coords_Slice_id, OMstr_to_name("dec"), &dec) != 1) 
		dec = 0;
	
	_3D_Coords_id = OMfind_subobj(Coords_Slice_id, OMstr_to_name("_3D_Coords"),OM_OBJ_RW);
	...
	OMset_name_int_val(Coords_Slice_id, OMstr_to_name("Dimension"), Dimension);
	
	xy_out = (float *)OMret_name_array_ptr(Coords_Slice_id, OMstr_to_name("xy_out"), OM_GET_ARRAY_WR, &xy_out_size, NULL);
