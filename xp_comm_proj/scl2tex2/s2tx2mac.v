...
flibrary Scalar2Texture2Macs
{
   ...
   macro Scalar2Texture2Func {
      ilink in_fld;
      ilink in_image;

      XP_COMM_PROJ.Scalar2Texture2.Scalar2Texture2Mods.Scalar2Texture2Core Scalar2Texture2Core {
         in => <-.in_fld;
      };

      FLD_MAP.combine_mesh_data combine_mesh_data {
         in_mesh => <-.in_fld;
         in_nd => <-.Scalar2Texture2Core.out;
         DataObject {
            texture_in<NEportLevels={3,1}> => <-.<-.in_image;
            Obj.name => name_of(<-.<-.<-.<-);
         };
      };
