...
int
cropdat3(OMobj_id cropdata_id, OMevent_mask event_mask, int seq_num)
{
   /***********************/
   /*  Declare variables  */
   /***********************/
...
OM_OBJ_RD); /*This is used to see if the mouse has moved and the value changed*/
  ...
  if ((set_coord == 2) && (which_view == 1) && ((OMchanged(min_in_id, seq_num) ==
1) || (OMchanged(max_in_id, seq_num) == 1)))
   ...
   /*fill in array min_axis1*/
      ...
      min_axis1[0]=min_in[0];
      min_axis1[1]=min_in[1];
   ...
   /*fill in array max_axis1*/
      ...
      max_axis1[0]=max_in[0];
      max_axis1[1]=max_in[1];
