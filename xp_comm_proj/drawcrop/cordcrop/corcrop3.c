...
int
coordcrop3(OMobj_id crop_coordinate_id, OMevent_mask event_mask, int seq_num)
{
   /***********************/
   /*  Declare variables  */
   /***********************/
   ...
   min_vec_axis0 = (float *)OMret_name_array_ptr(crop_coordinate_id, OMstr_to_name("min_vec_axis0"), OM_GET_ARRAY_RD,
			&min_vec_axis0_size, NULL);

   max_vec_axis0 = (float *)OMret_name_array_ptr(crop_coordinate_id, OMstr_to_name("max_vec_axis0"), OM_GET_ARRAY_RD,
			&max_vec_axis0_size, NULL);
   ...
   if (set_coord == 3)
     {
       /*printf("coordcrop3 is running and set_coord = %d \n",set_coord);*/
       ...
       xmin=stored[0];
       ymin=stored[1];
       zmin=(int)   (min_vec_axis0[1]);
       
       xmax=stored[2];
       ymax=stored[3];
       zmax=(int)   (max_vec_axis0[1]);
   ...
   if (min_vec_axis0 != NULL) 
      ARRfree((char *)min_vec_axis0);

   if (max_vec_axis0 != NULL) 
      ARRfree((char *)max_vec_axis0);
