...
char		infile_name[STRING_LEN];// input file name (xxx.ann)
...
{
    if (level == ANNabort) {
        cerr << "test: ERROR------->" << msg << "<-------------ERROR\n";
        exit(1);
    }
    else {
        cerr << "test: WARNING----->" << msg << "<-------------WARNING\n";
    }
}
...
//	    file		file (input=file.ann, output=file.fig)
//
//----------------------------------------------------------------------
...
void readANN()
{
	...
	float box_len_x = bnd_box.hi[dim_x] - bnd_box.lo[dim_x];
	float box_len_y = bnd_box.hi[dim_y] - bnd_box.lo[dim_y];
