...
//	in kd_tree.cc for more information.
//----------------------------------------------------------------------
	...
	ostream &out)			// output stream
{
    ...
    child[IN]->print(level+1, out);		// print in-child
}

//----------------------------------------------------------------------
//  Dumping a bd-tree
//	This routine dumps a bd-tree to a file.  See file kd_tree.cc
//	for information on dump format.  The added feature here is that
//	there is a new node called a "shrink" node.
//
//	Format:
//	Shrinking nodes:
//		shrink <n_bnds>
	...
	ostream &out)			// output stream
{
    ...
    child[IN]->dump(out);			// print in-child
    child[OUT]->dump(out);			// print out-child
}

//----------------------------------------------------------------------
//  kd_tree statistics utility  (for performance evaluation)
//	This routine computes various statistics information for
//	shrinking nodes.  See file kd_tree.cc for more information.
//----------------------------------------------------------------------
    ...
    ANNorthRect		&bnd_box)		// bounding box
{
    ...
    child[IN]->getStats(dim, ch_stats, inner_box);
    st.merge(ch_stats);				// merge them
						// get stats for outer child
    ch_stats.reset();				// reset
    child[OUT]->getStats(dim, ch_stats, bnd_box);
    st.merge(ch_stats);				// merge them
...
//
//	We repeatedly apply the splitting rule, always to the larger
//	subset of points, until the number of points decreases by the
//	constant fraction BD_FRACTION.  If this takes more than
//	BD_MAX_SPLIT splits for this to happen, then we shrink to the
//	final inner box Otherwise we split.
//----------------------------------------------------------------------

const float	BD_MAX_SPLIT = 4;	// maximum number of splits allowed
    ...
    ANNorthRect		&inner_box)	// inner box if shrinking (returned)
{
    ...
    if (n_splits > BD_MAX_SPLIT)	// took too many splits
...
ANNdecomp selectDecomp(		// select decomposition method
    ...
    ANNorthRect		&inner_box)	// inner box if shrinking (returned)
{
    ANNdecomp decomp = SPLIT;			// decomposition (plus assignement to placate the compiler)
...
//	procedure rkd_tree() in kd_split.cc for more information.
//
