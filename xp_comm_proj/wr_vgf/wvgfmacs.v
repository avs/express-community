...
flibrary WriteVGFMacs <compile_subs=0> {
 ...
 macro WriteVGF_UI {
	...
	UImod_panel panel {
            title => name_of(<-.<-.<-,1);
	    message = "Select Write VGF control panel.";
	...
	string+OPort2 filename = "/tmp/avs.vgf";
    };


   // Writes a VGF file based upon a single field input
   macro writeVGF {
      ...
      XP_COMM_PROJ.WriteVGF.WriteVGFMods.WriteVGFCore WriteVGFCore {
         filename => <-.WriteVGF_UI.filename;
         in => {<-.in_fld};
         nobjs = 1;
         top_name => <-.name;
      };

      WriteVGF_UI WriteVGF_UI {
         err_vis => <-.WriteVGFCore.err;
         err_msg => <-.WriteVGFCore.err_str;
      };
   ...
   // Writes a VGF file based upon a single Graphic Display Object
   writeVGF writeVGF_GD {
   ...
   // Writes a VGF file based upon a GroupObject (a group of Graphic Display Objects)
   writeVGF writeVGF_ARR {
      ...
      GDM.GDobject_templ+nres &obj_list<NEportLevels={1,1}>[] => top_obj.objects;
   ...
   // Demonstrates how a VGF file can be created from a field input
   APPS.SingleWindowApp WriteVGFEg {
      ...
      writeVGF writeVGF_scalar<NEx=121.,NEy=253.> {
   ...
   // Demonstrates how a VGF file can be created from a GroupObject input
   APPS.MultiWindowApp WriteVGF_ARR_Eg {
      ...
      writeVGF_ARR writeVGF_ARR<NEx=484.,NEy=231.> {
